...
class RecallRepository:
    ...
    def insert(self, role: str, content: str, embedding: Optional[bytes] = None,
               tools_used: list = None, metadata: dict = None) -> int:
    ...
    def update_embedding(self, row_id: int, embedding: bytes) -> None:
        with _connect_db(self.db_path) as conn:
            conn.execute("UPDATE recall_memory SET embedding = ? WHERE id = ?", (embedding, row_id))
...
class ArchivalRepository:
    ...
    def insert(self, category: str, content: str, importance: int = 5,
               embedding: Optional[bytes] = None, metadata: dict = None) -> int:
        with _connect_db(self.db_path) as conn:
            cursor = conn.execute(
