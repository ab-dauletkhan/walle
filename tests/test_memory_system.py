...
class TestBackpressureAndShutdown(unittest.TestCase):
    """Tests for embedding queue backpressure and graceful shutdown."""

    def setUp(self):
        self.temp_dir = tempfile.mkdtemp()
        self.db_path = os.path.join(self.temp_dir, "test_bp.db")
    ...
    def test_backpressure_skips_when_full(self):
        ...
        with recall._pending_lock:
            for _ in range(2):
                f = recall._embedding_pool.submit(blocking_task)
                recall._pending_futures.add(f)

        # This insert should skip the deferred embedding (queue full)
        recall.insert("user", "This should skip embedding", defer_embedding=True)

        with recall._pending_lock:
            # Should still be exactly 2 (the blocking ones), not 3
            self.assertEqual(len(recall._pending_futures), 2)

        # Cleanup
        block_event.set()
        recall._MAX_PENDING_EMBEDDINGS = original_max
        recall.shutdown(timeout=5)

    def test_shutdown_with_inflight_embeddings(self):
        """Shutdown should complete cleanly with in-flight embedding tasks."""
        recall = RecallMemory(db_path=self.db_path, use_semantic=True)
    ...
    def test_callback_removes_from_pending(self):
        """Completed futures should be removed from _pending_futures via callback."""
        recall = RecallMemory(db_path=self.db_path, use_semantic=True)

        recall.insert("user", "Test callback cleanup", defer_embedding=True)

        # Wait briefly for the embedding to complete
        time.sleep(3)

        with recall._pending_lock:
            # Future should have been removed by callback
            self.assertEqual(len(recall._pending_futures), 0)
...
def run_all_tests():
    ...
    suite.addTests(loader.loadTestsFromTestCase(TestBackpressureAndShutdown))
