...
#include "Queue.hpp"
#include "L298NMotorController.hpp"


/// Define pin-mapping
// -- -- -- -- -- -- -- -- -- -- -- -- -- --
// L298N pins aligned with working sketch:
//   Motor A: ENA=3, IN1=2, IN2=4
//   Motor B: ENB=6, IN3=5, IN4=7
#define L298N_L_IN1 2                // L298N Left motor IN1 (crew IN1)
#define L298N_L_IN2 4                // L298N Left motor IN2 (crew IN2)
#define L298N_L_ENA 3                // L298N Left motor ENA (PWM)
#define L298N_R_IN1 5                // L298N Right motor IN1 (crew IN3)
#define L298N_R_IN2 7                // L298N Right motor IN2 (crew IN4)
#define L298N_R_ENA 6                // L298N Right motor ENA (PWM)
#define SERVO_ENABLE_PIN 11          // PCA9685 OE (LOW=enabled) per crew sketch
...
// Set up L298N motor controller classes (no brake pin)
L298NMotorController motorL(L298N_L_IN1, L298N_L_IN2, L298N_L_ENA);
L298NMotorController motorR(L298N_R_IN1, L298N_R_IN2, L298N_R_ENA);
...
float prevpos[NUMBER_OF_SERVOS];                                    // Previous position for stuck detection
unsigned long stuckTimer[NUMBER_OF_SERVOS];                         // Timestamp of last position change
bool servoDisabled[NUMBER_OF_SERVOS];                               // Stuck servo disabled flag
...
void setup() {
	...
	// Initialize L298N motor drivers
	motorL.init();
	motorR.init();
...
void evaluateSerial() {
	...
	// -- -- -- -- -- -- -- -- -- -- -- -- -- --
		...
		// Report servo positions and motor state for Jetson health monitoring
		Serial.print(F("STATUS "));
		for (int i = 0; i < NUMBER_OF_SERVOS; i++) {
			Serial.print(int(curpos[i]));
			if (i < NUMBER_OF_SERVOS - 1) Serial.print(',');
		}
		Serial.print(F(" M"));
		Serial.print(int(curvel[NUMBER_OF_SERVOS]));
		Serial.print(',');
		Serial.println(int(curvel[NUMBER_OF_SERVOS + 1]));
	}

	// ACK — tell the host that the command was accepted and applied.
	// The Python serial_manager waits for this before returning "OK".
	Serial.println(F("OK"));
}
...
void manageServos(float dt) {
	...
	for (int i = 0; i < NUMBER_OF_SERVOS; i++) {
		...
		if (abs(posError) > CONTROLLER_THRESHOLD && (setpos[i] != -1)) {
			...
			// Stuck detection: if position hasn't changed for SERVO_STUCK_TIMEOUT, disable
			if (abs(curpos[i] - prevpos[i]) > 0.5) {
				prevpos[i] = curpos[i];
				stuckTimer[i] = millis();
				servoDisabled[i] = false;
			} else if (!servoDisabled[i] && (millis() - stuckTimer[i] >= SERVO_STUCK_TIMEOUT)) {
				servoDisabled[i] = true;
				curvel[i] = 0;
				pwm.setPin(i, 0);  // Disable stuck servo to prevent overcurrent
				Serial.print(F("WARN: Servo ")); Serial.print(i); Serial.println(F(" stuck, disabled"));
				continue;
			}
			if (servoDisabled[i]) continue;
...
void manageMotors(float dt) {
	...
	// Snapshot volatile values to prevent mid-update changes from serial interrupt
	int localMove = moveValue;
	int localTurn = turnValue;
	int localTrim = turnOffset;

	// Update Main Motor Values
	setpos[NUMBER_OF_SERVOS] = localMove - localTurn;
	setpos[NUMBER_OF_SERVOS + 1] = localMove + localTurn;

	// Apply turn offset (motor trim) only when motors are active
	if (setpos[NUMBER_OF_SERVOS] != 0) setpos[NUMBER_OF_SERVOS] -= localTrim;
	if (setpos[NUMBER_OF_SERVOS + 1] != 0) setpos[NUMBER_OF_SERVOS + 1] += localTrim;
