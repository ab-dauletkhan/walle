...
class ContextManager:
    ...
    def update_visual(self, visual_context: VisualContext):
        """Update visual context from camera/vision system (thread-safe)."""
        with self._lock:
            self.visual = visual_context
            self._generate_observations()

    def update_environment(self, env_context: EnvironmentContext):
        """Update environment context from sensors (thread-safe)."""
        with self._lock:
    ...
    def get_context_string(self) -> str:
        ...
        """
        with self._lock:
            return self._get_context_string_unlocked()

    def _get_context_string_unlocked(self) -> str:
