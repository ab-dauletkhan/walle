...
function showAlert(error, bold, content, fade) {
	...
	var $alert = $('<div class="alert alert-dismissible set-alert"></div>').addClass(alertType);
	$alert.append($('<button type="button" class="close" data-dismiss="alert"></button>').html('&times;'));
	$alert.append($('<strong></strong>').text(bold));
	$alert.append(' ');
	$alert.append($('<span></span>').text(content));
	$('#alert-space').html($alert);			
	if (!alertVisible) 
	{
