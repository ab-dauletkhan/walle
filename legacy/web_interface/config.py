...
# Secret key used for login session cookies — override via WALLE_SECRET_KEY env var
_env_key = os.environ.get("WALLE_SECRET_KEY", "")
if _env_key:
    ...
    _logging.warning(
        "WALLE_SECRET_KEY not set; using random key (sessions won't persist across restarts)"
    )

# Web Interface Settings
APP_PORT = 5000  # Port of the application
APP_DEBUG = False  # Enable / Disable Python Server Debugging
LOGIN_PASSWORD = os.environ.get("WALLE_PASSWORD", "walle")  # Password for web-interface
if LOGIN_PASSWORD == "walle":
    import logging as _logging

    _logging.warning(
        "Using default password 'walle'. Set WALLE_PASSWORD env var for production."
    )
ARDUINO_PORT = "/dev/ttyACM0"  # Default port which will be selected
AUTOSTART_ARDUINO = (
    True  # False = no auto connect, True = automatically try to connect to default port
)
AUTOSTART_CAM = True  # False = no auto start, True = automatically start up the camera
SOUND_FOLDER = os.path.join(
    BASEDIR, "static/sounds/"
)  # Location of the folder containing all audio files
...
RB_CMD = [
    ...
    "1.8",
    "-q",
]  # Rubberband for pitch shifting TTS
