...
def _play_audio_stable(audio: np.ndarray, sample_rate: int) -> None:
...
class Mimic3TTSEngine(BaseTTSEngine):
    ...
    def speak(self, text: str) -> None:
        try:
            ...
            print(f"  TTS error: cannot connect to Mimic3 at {self._url}", file=sys.stderr)
...
class SpeechRouter(TranscriptEventListener):
    ...
    _ECHO_COOLDOWN = 2.0
    _ECHO_WINDOW = 8.0  # seconds after TTS to keep content-based filtering
    ...
    def _detect_wake(self, text: str) -> Optional[str]:
        ...
        wake_tokens = set(wake_clean.split())

        if wake_clean in clean:
            idx = clean.index(wake_clean) + len(wake_clean)
            return self._strip_punctuation(text)[idx:].strip()
        if wake_tokens.issubset(set(clean.split())):
            return ""
        return None
